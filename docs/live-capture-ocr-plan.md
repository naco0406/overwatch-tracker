...
이 기능은 홈의 빠른 수기 기록을 대체하지 않는다. 메인 입력 플로우는 계속 빠른 수기 기록이고, Live는 웹서비스 안에서 맵 선택 추천과 경기 종료 후 기록 보조를 담당한다.
...
- 게임 위에 HUD를 띄우지 않는다. 순수 웹 MVP는 오버워치 클라이언트 위가 아니라 `/live` 화면과 GNB 상태 영역에만 정보를 표시한다.
...
결론: 사용자가 GNB 하단의 화면 공유 버튼을 누르고 브라우저 화면 공유 선택창에서 오버워치 창/모니터를 직접 선택하는 흐름이면 웹만으로 시작할 수 있다. 자동 창 탐색, 백그라운드 무단 추적, 안정적인 전체화면 독점 캡처가 필요해지는 시점에만 Electron/네이티브 앱을 재검토한다.
...
- 기본 프레임 진단: 5초 간격
- 화면 상태 변화 감지: 3-5초 간격
- 이미지 매칭/맵 후보 탐지: 맵 선택 후보일 때 3-5초 간격
- OCR: 결과/점수판 후보가 안정적으로 유지될 때 5-10초 간격 또는 짧은 burst
- 결과 화면 감지 시: 2-3개 샘플을 비교해 최고 confidence 초안 생성
...
현재 `AppLayout`은 좌측 GNB와 모바일 하단 내비게이션을 가진다. Live는 PC 환경 전용 기능이므로 데스크톱 좌측 GNB 하단에만 상시 진입점을 둔다.

- 데스크톱: 좌측 GNB 하단 계정 영역 위에 `LIVE` 메뉴와 화면 공유 버튼을 항상 둔다.
- 모바일: LIVE 메뉴와 화면 공유 트리거를 노출하지 않는다.
- 캡처 시작 전: `LIVE` 메뉴는 대기 상태로 `/live` 대기 화면에 진입한다.
- 캡처 시작 성공 후: 같은 GNB 영역이 빨간 LIVE 상태로 바뀌고 `/live`에서 프레임/추천/초안을 보여준다.
- 캡처 중단 후: `/live`에 남아 대기 상태를 보여주며 버튼은 다시 `화면 공유`로 돌아간다.

### Live page states

`/live`는 세션 상태를 중심으로 단순하게 구성한다.

- `idle`: 데스크톱 화면 공유 버튼과 짧은 개인정보/권한 안내.
...
- 상단 상태: LIVE, 공유 대상 타입, 샘플 간격, 마지막 감지 시각.
- 맵 선택 화면: 현재 후보 맵과 내 데이터 기준 추천을 가장 먼저 보여준다.
- 결과/점수판 화면: 저장 후보 초안을 가장 먼저 보여준다.
- 기타 진행 중 화면: 무거운 OCR을 쉬고 현재 컨텍스트와 최근 증거만 낮은 밀도로 보여준다.
- 최근 감지 로그, confidence, raw evidence는 기본 화면을 어지럽히지 않도록 보조 영역이나 접힘 영역에 둔다.
...
v1 필수 필드는 맵, 모드, 결과, 우리/상대 스코어다. 그 외 정보는 confidence가 높을 때만 보조 evidence로 둔다.
...
라우터와 GNB는 Provider 상태를 구독한다. 데스크톱 GNB 하단에는 LIVE 메뉴를 항상 노출하고, `running | muted` 상태에서만 빨간 LIVE 상태와 종료 버튼을 보여준다. 모바일 GNB/하단 내비게이션에는 노출하지 않는다.
...
- `screen-state`: 3000-5000ms
- `map-candidates`: 맵 선택 후보일 때 3000-5000ms
- `result-ocr`: 결과 후보일 때 5000-10000ms 또는 짧은 burst
...
- `active-match`는 추천이나 OCR 대상이 아니다. 매치 컨텍스트 유지와 결과 화면 전환 감지만 수행한다.
...
- GNB 하단 상시 `LIVE` 메뉴와 화면 공유 버튼 추가.
- 캡처 상태에 따라 LIVE 색상, 상태 라벨, 화면 공유/공유 종료 버튼 전환.
- `/live` 라우트와 상태별 기본 화면 추가.
- 모바일에서 LIVE 메뉴와 시작/중단 컨트롤을 노출하지 않는다.

완료 기준:

- 데스크톱에서는 어느 페이지에서든 화면 공유 시작/중단이 가능하다.
...
- `active-match`는 무거운 분석을 실행하지 않는 유지 상태로만 둔다.
