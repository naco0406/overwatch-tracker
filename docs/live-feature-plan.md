...
LIVE는 오버워치 클라이언트 화면을 사용자가 직접 공유하도록 요청하고, 공유된 화면에서 프레임을 꾸준히 샘플링해 한 경기의 맥락을 누적하는 기능이다. 최종 목표는 단일 스크린샷 한 장에 의존하지 않고, 경기 진행 중 관찰한 맵, 모드, 스코어, 결과 화면 증거를 합쳐 기록 정확도를 높이는 것이다.
...
1. 사용자가 데스크톱 GNB 하단의 `LIVE 시작` 영역에서 `화면 공유`를 누른다.
2. 오버워치 창 또는 화면을 선택한다.
3. 화면 공유 스트림이 실제로 연결되면 데스크톱 GNB 하단 영역이 빨간 LIVE 메뉴로 바뀐다.
4. LIVE 메뉴는 공유 스트림이 활성일 때만 표시한다. 평상시에는 일반 메뉴에 LIVE를 노출하지 않는다.
...
7. 각 프레임은 가벼운 분석부터 수행한다.
   - 해상도, 프레임 수, 샘플 시간
   - 평균 밝기, 대비, 프레임 읽힘 여부
   - 이후 화면 상태 분류: 맵 선택, 경기 중 HUD, 점수판, 결과/진척도
...
- LIVE 라우트와 데스크톱 GNB 하단 트리거 추가
- 화면 공유가 연결된 동안에만 데스크톱 GNB 하단 트리거를 빨간 LIVE 메뉴로 전환
- 화면 공유 시작/중지
...
- 맵 선택 화면으로 판단되면 현재 모드 또는 전체 기준 맵별 승률을 추천한다.
