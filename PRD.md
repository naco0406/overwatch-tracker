...
- 각 탭은 기록이 1개 이상 있는 축만 표시한다. 영웅 미지정 경기는 전체/모드/맵 승률에는 포함하고 영웅별 통계에서만 제외한다.
- 개인 수천 건 규모는 클라이언트 집계로 유지한다. 10,000건 이상 또는 통계 초기 로드 1초 초과가 반복되면 Supabase RPC/일별 집계 테이블로 이전한다.
